...
class MomentObserver:
    ...
    def __init__(self, number_timesteps, delta_x=1):
        self.number_timesteps = number_timesteps
        self.deltax = delta_x
    ...
    def calculate_moments(self, quantity, timestep):
        ...
        self.mass_array[timestep] = np.sum(quantity) * self.deltax
        self.variance_array[timestep] = np.sum((quantity - self.mass_array[timestep])**2)*self.deltax
