...
if __name__ == "__main__":
    ...
    for advection_scheme_key in used_advection_schemes:
        ...
        plt.loglog(scaled_dx_array, error_array, ".-", color=color_dic[advection_scheme_key], marker=next(marker),
    ...
    #  is decreasing)
    plt.loglog(scaled_dx_array, scaled_dx_array / scaled_dx_array[-1] * error_dict["BTCS"][-1], "--",
               label=r"$\sim\Delta x^1$")

    plt.loglog(scaled_dx_array, scaled_dx_array / scaled_dx_array[-1] * error_dict["FTBS"][-1], "--",
               label=r"$\sim\Delta x^1$", color="C2")
